...
export default function BankRequestsTable() {
    ...
    const [requests, setRequests] = useState<BankRequestItem[]>([]);
    const [loading, setLoading] = useState(true);
    const [search, setSearch] = useState("");
    const [statusFilter, setStatusFilter] = useState<RequestStatus | "all">("all");
    const [selected, setSelected] = useState<BankRequestItem | null>(null);
    const [detailStatus, setDetailStatus] = useState<RequestStatus>("pending");
    ...
    const confirmDelete = async () => {
        ...
        try {
            ...
            await axios.delete(
                `${process.env.NEXT_PUBLIC_API_URL}api/bank-requests/${deleteId}`,
                { withCredentials: true }
            );
    ...
    return (
        <div className="bg-[#0f2234] border border-white/5 rounded-3xl p-6 shadow-2xl">
            ...
            <ConfirmModal
                ...
                description="This will permanently remove the bank request. This action cannot be undone."
                confirmText="Delete"
                loading={deleting}
                    ...
                    <div className="space-y-5">
                        ...
                        <div>
                            ...
                            <p className="text-sm text-gray-700 bg-gray-50 border border-gray-200 rounded-xl p-3 whitespace-pre-wrap">
                                {selected.additionalContext || "No additional notes provided."}
                            </p>
                        </div>
