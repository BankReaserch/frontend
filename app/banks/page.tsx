...
const filtered = useMemo(() => {
  ...
  const SortIcon = ({ k }: { k: typeof sortKey }) => {
    ...
    return sortDir === 1 ? <ChevronUp className="h-3 w-3 text-[#c8a21a]" /> : <ChevronDown className="h-3 w-3 text-[#c8a21a]" />;
  };
    ...
    <>
      ...
      <main className="min-h-screen bg-[#f5f1ea] overflow-x-hidden">
        ...
        <section className="relative bg-[#051933] overflow-hidden">
          <div className="pointer-events-none absolute inset-0 opacity-[0.035] bg-[repeating-linear-gradient(135deg,#fff_1px,transparent_1px)] bg-size-[28px_28px]" />
          <div className="pointer-events-none absolute left-1/4 -top-20 h-125 w-125 -translate-x-1/2 rounded-full bg-[#c8a21a]/8 blur-3xl" />
            ...
            <div className="grid lg:grid-cols-[1fr_380px] gap-12 items-start">
              ...
              <div>
                ...
                <p className="text-[15px] leading-8 text-[#94a3b8] max-w-lg mb-4">
                  Comprehensive research on the kashrus status of financial institutions
                  across the United States, guided by leading poskim.
                </p>
              ...
              <div className="rounded-2xl overflow-hidden border border-white/10 bg-white/[0.04]">
                ...
                {(Object.entries(STATUS_CFG) as [Status, (typeof STATUS_CFG)[Status]][]).map(([key, cfg]) => (
                  <div key={key} className="flex items-start gap-3 px-5 py-3 border-b border-white/5 last:border-none hover:bg-white/[0.03] transition-colors">
        ...
        <section className="py-14 pb-24">
          <div className="mx-auto max-w-6xl px-6">
            ...
            <div className="flex flex-wrap gap-2 mb-8">
              ...
              {(Object.entries(STATUS_CFG) as [Status, (typeof STATUS_CFG)[Status]][]).map(([key, cfg]) => (
                <button key={key} onClick={() => { setFilter(key); }}
                  className={`inline-flex items-center gap-1.5 rounded-full px-4 py-2 text-[12px] font-semibold transition-all ${filter === key ? `${cfg.badgeBg} ${cfg.badgeText} ring-2 ring-current/20` : "border border-[#e8e2d6] bg-white text-[#64748b] hover:border-gray-300"}`}
                >
            ...
            <div className="flex items-center justify-between mb-5">
              <p className="text-[11px] font-semibold uppercase tracking-[0.15em] text-[#94a3b8]">
                {filtered.length} bank{filtered.length !== 1 ? "s" : ""}
              </p>
            </div>

            {/* Main layout: content + detail panel */}
            <div className={`grid gap-5 items-start ${selected ? "lg:grid-cols-[1fr_340px]" : "grid-cols-1"}`}>
                          ...
                          {filtered.map((bank) => (
                            ...
                            >
                              <td className="px-5 py-4">
                                ...
                                <p className="text-[11px] text-[#94a3b8] mt-0.5">{bank.hq} · Est. {bank.founded}</p>
                              </td>
                              <td className="px-5 py-4 text-[12px] text-[#64748b]">
                                {bank.type}
                                <p className="text-[11px] text-[#b0a898] mt-0.5">{bank.assets}</p>
                              </td>
                              <td className="px-5 py-4"><StatusBadge status={bank.status} /></td>
                            </tr>
                          ))}
                        </tbody>
                      </table>
