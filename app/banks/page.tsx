...
//                                   >
//                                     {k === "name" ? "Bank Name" : k === "Date" ? "Date" : "Kashrus Status"}
//                                     <SortIcon k={k} />
//                                   </button>
//                                 </th>
//                               ))}
...
//                                     {bank?.lastReviewed
//                                       ? new Date(bank.lastReviewed).toLocaleDateString("en-IN", {
//                                         day: "2-digit",
//                                         month: "short",
//                                         year: "numeric",
//                                       })
...
export default function BanksPage() {
  ...
  const [banks, setBanks] = useState<BankType[]>([]);
  const [loading, setLoading] = useState(true);
  const [previewBank, setPreviewBank] = useState<BankType | null>(null);
  ...
  const filtered = useMemo(() => {
    ...
    data.sort((a, b) => {
      const av = String(sortKey === "Date" ? a.lastReviewed ?? "" : a[sortKey] ?? "");
      const bv = String(sortKey === "Date" ? b.lastReviewed ?? "" : b[sortKey] ?? "");
  ...
  // Same gate the list view already applies: try the authenticated report
  // first, and if the server says no active plan (401/403), fall back to
  // the blurred single-page preview + upgrade CTA instead of a raw error.
  const handleViewReport = async (bank: BankType) => {
    if (!bank.reportUrl) return;

    try {
      await axios.head(bank.reportUrl, { withCredentials: true });
      window.open(bank.reportUrl, "_blank", "noopener,noreferrer");
    } catch {
      setPreviewBank(bank);
    }
  };
  ...
  const SkeletonRow = () => (
    <tr className="border-b border-[#f1ede6] animate-pulse">
      ...
      <td className="px-5 py-4"><div className="h-3 bg-[#e8e2d6] rounded-full w-24" /></td>
      <td className="px-5 py-4"><div className="h-6 bg-[#e8e2d6] rounded-full w-20" /></td>
      <td className="px-5 py-4 text-right"><div className="h-3 bg-[#e8e2d6] rounded-full w-16 ml-auto" /></td>
    </tr>
  );
  ...
  return (
    <>
      ...
      {previewBank && (
        <ReportPreviewModal
          bankName={previewBank.name}
          previewUrl={`${process.env.NEXT_PUBLIC_API_URL}api/banks/preview-report/${previewBank._id}`}
      ...
      <main className="min-h-screen bg-[#f5f1ea] overflow-x-hidden">
        ...
        <section className="py-14 pb-24">
          <div className="mx-auto max-w-6xl px-6">
            ...
            <div
              className={`grid gap-5 items-start transition-all duration-300 ${view === "list" && selected ? "lg:grid-cols-[1fr_360px]" : "grid-cols-1"
                }`}
            >
              <div>
                {/* ── GRID VIEW ── */}
                    ...
                    {loading ? (
                      <div className="overflow-hidden rounded-2xl border border-[#e8e2d6] bg-white">
                        <table className="w-full border-collapse">
                          <thead>
                            <tr className="border-b border-[#e8e2d6] bg-[#fafaf8]">
                              {["Bank Name", "Date", "Kashrus Status", ""].map((h) => (
                                <th key={h} className="px-5 py-3 text-left text-[9px] font-semibold uppercase tracking-[0.18em] text-[#94a3b8]">{h}</th>
                              ))}
                            </tr>
                          </thead>
                        ...
                        <table className="w-full border-collapse">
                          <thead>
                            <tr className="border-b border-[#e8e2d6] bg-[#fafaf8]">
                              ...
                              {/* Fourth column mirrors the grid card's "View More" affordance,
                                  so both views teach the same interaction: rows/cards open the
                          ...
                          <tbody>
                            {filtered.map((bank) => {
                              const isSelected = selected?._id === bank._id;
                              return (
                                <tr
                                  key={bank._id}
                                  onClick={() => handleSelect(bank)}
                                  className={`group cursor-pointer border-b border-[#f1ede6] last:border-none transition-colors ${isSelected
                                    ? "bg-[#fffbf0] border-l-[3px] border-l-[#c8a21a]"
                                    : "hover:bg-[#fdfaf4]"
                                    }`}
                                >
                                  ...
                                  <td className="px-5 py-4">
                                    <span
                                      className={`flex items-center justify-end gap-1.5 text-[11px] font-medium transition-colors ${isSelected
                                        ? "text-[#8b6b00]"
                                        : "text-[#94a3b8] group-hover:text-[#8b6b00]"
                                        }`}
                                    >
                                      {isSelected ? "Selected" : "View More"}
                                      <ArrowRight
                                        className={`h-3.5 w-3.5 transition-transform ${isSelected ? "translate-x-0.5" : "group-hover:translate-x-0.5"
                                          }`}
                                      />
                                    </span>
                                  </td>
              ...
              {/* Right column: sticky detail panel — list view only. Grid
                  cards show their full profile inline instead. */}
              {view === "list" && selected && (
                <div ref={detailRef} className="lg:sticky lg:top-6">
                  <DetailPanel bank={selected} onClose={() => setSelected(null)} />
                </div>
